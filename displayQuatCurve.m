function env = displayQuatCurve( r, q, axiS, axis_order )
...
%DATE       19.06.2012
    ...
    set(0, 'CurrentFigure', axiS.Parent );
...
function displayQuats(r,q,i,range,axis_order)
        ...
        z = z(axis_order);
        dir_origin = r(i,:);
        arrow_length_scaling = 1;
        arrow_head_size = 50;
        arrow_length = range * arrow_length_scaling;
        arrow(dir_origin, dir_origin + (arrow_length .* x) / norm(x), arrow_head_size, 'EdgeColor','b','FaceColor','b','LineWidth',2);
        arrow(dir_origin, dir_origin + (arrow_length .* y) / norm(y), arrow_head_size, 'EdgeColor','g','FaceColor','g','LineWidth',2);
        arrow(dir_origin, dir_origin + (arrow_length .* z) / norm(z), arrow_head_size, 'EdgeColor','r','FaceColor','r','LineWidth',2);
end
